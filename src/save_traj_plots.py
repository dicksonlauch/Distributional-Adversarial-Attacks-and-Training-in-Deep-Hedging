...
def save_plots():
    ...
    ax.set_title('Crash Scenario')
    lines = l1 + l2 + l3 + l4
    labels = [l.get_label() for l in lines]
    ax.legend(lines, labels, loc='upper right', bbox_to_anchor=(1.35, 1))
    plt.tight_layout()
    plt.savefig('traj_crash.png')

    # 2. Up Scenario
    idx_u = np.argmax(rets)
    S_path = S[idx_u, :-1]
    ...
    ax.set_ylabel('Stock Delta', color='g')
    ax2.set_ylabel('VIX Hedge', color='purple')
    ax.set_title('Up Scenario')
    lines = l1 + l2 + l3 + l4
    labels = [l.get_label() for l in lines]
    ax.legend(lines, labels, loc='upper right', bbox_to_anchor=(1.35, 1))
    plt.tight_layout()
    plt.savefig('traj_up.png')

    # 3. Pin Risk Scenario
    idx_p = np.argmin(np.abs(S[:, -1] - K))
    S_path = S[idx_p, :-1]
    ...
    ax.set_ylabel('Stock Delta', color='g')
    ax2.set_ylabel('VIX Hedge', color='purple')
    ax.set_title('Pin Risk Scenario')
    lines = l1 + l2 + l3 + l4
    labels = [l.get_label() for l in lines]
    ax.legend(lines, labels, loc='upper right', bbox_to_anchor=(1.35, 1))
    plt.tight_layout()
    plt.savefig('traj_pin.png')
